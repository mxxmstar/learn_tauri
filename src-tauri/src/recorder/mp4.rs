//! MP4 录像器实现
//!
//! 使用 FFI 对接 C++/FFmpeg 实现 MP4 封装（支持 H.264/H.265）
...
/// MP4 录像器
///
/// 封装 FFI 接口，提供 MP4 封装功能（支持 H.264/H.265）
///
/// # 示例
///
/// ```no_run
/// use learn_tauri_lib::recorder::{Mp4Recorder, Recorder, RecorderConfig};
...
impl Mp4Recorder {
    ...
    pub fn new(codec_type: CodecType, output_path: PathBuf) -> RecordResult<Self> {
        ...
        let container_format_i32 = super::config::ContainerFormat::MP4 as i32;
    ...
    pub fn get_info(&self) -> super::trait_::RecorderInfo {
        super::trait_::RecorderInfo {
            name: "MP4 Recorder (FFI)".to_string(),
            supported_codecs: vec![CodecType::H264, CodecType::H265],
...
mod tests {
    ...
    #[test]
    fn test_mp4_recorder_invalid_codec() {
        // 测试使用不支持的编解码器创建 MP4 录像器
        // 此测试不调用 FFI，仅验证编解码器支持检查
        let result = Mp4Recorder::new(
            CodecType::MJPEG,  // MJPEG 不支持 MP4 封装
