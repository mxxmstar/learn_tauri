...
async fn sftp_download_file_inner(
    ...
    let final_local_path = PathBuf::from(local_path);

    if final_local_path.as_os_str().is_empty() {
    ...
    let metadata = fs::metadata(&saved_path)
        .await
        .with_context(|| format!("读取本地文件属性失败：{}", saved_path.display()))?;
...
async fn sftp_upload_file_inner(
    ...
    let file_name = local_path
        ...
        .ok_or_else(|| anyhow!("无法从本地路径中解析文件名"))?;
    ...
    upload_local_file_to_remote(&session, session_id, &local_path, &remote_path, window).await?;

    let metadata = fs::metadata(&local_path)
        .await
        .with_context(|| format!("读取本地文件属性失败：{}", local_path.display()))?;
...
async fn sftp_open_text_file_inner(
    ...
    let temp_cache_path = build_temp_local_path(&cache_path);
    let saved_path = download_remote_file_to_local(None, session_id, &remote_path, &cache_path, &temp_cache_path).await?;

    let metadata = fs::metadata(&saved_path)
        .await
        .with_context(|| format!("读取缓存文件属性失败：{}", saved_path.display()))?;

    let is_text = is_text_file(&remote_path);
    let text_content = if is_text {
        let content = fs::read(&saved_path)
            .await
            .with_context(|| format!("读取缓存文本内容失败：{}", saved_path.display()))?;
...
/// 将目录项转换为前端表格结构。
fn convert_dir_entry(parent_path: &str, entry: DirEntry) -> anyhow::Result<Option<RemoteFileEntry>> {
...
/// 计算进度比例。
fn calculate_progress(transferred_bytes: u64, total_bytes: u64) -> f64 {
    if total_bytes == 0 {
        0.0
    } else {
...
/// 构造下载 / 缓存过程中的临时文件路径。
fn build_temp_local_path(final_path: &Path) -> PathBuf {
