...
impl H265Decoder {
    ...
    pub fn new() -> DecodeResult<Self> {
        #[cfg(feature = "decoder-ffi")]
        {
            // FFI 实现：调用 C++ 端初始化解码器
            // TODO: 实现 FFI 调用
            return Err(DecodeError::UnsupportedCodec(CodecType::H265));
        }

        #[cfg(not(feature = "decoder-ffi"))]
        {
            Err(DecodeError::UnsupportedCodec(CodecType::H265))
        }
    }
...
impl Default for H265Decoder {
    fn default() -> Self {
        Self {
            output_format: PixelFormat::NV12, // H265 常用 NV12 格式输出
            initialized: false,
        }
    }
}
...
impl Decoder for H265Decoder {
    fn decode(&mut self, packet: &MediaPacket) -> DecodeResult<Option<MediaFrame>> {
        // 检查编解码器类型
        if packet.codec_type != CodecType::H265 {
            return Err(DecodeError::InvalidParameter(format!(
                "expected H265 codec, got {:?}",
                packet.codec_type
            )));
        }
...
mod tests {
    ...
    #[test]
    fn test_h265_decoder_new() {
        let result = H265Decoder::new();
        // 默认应该返回错误（未实现）
        assert!(result.is_err());
    }

    #[test]
    fn test_h265_decoder_codec_type() {
        // 由于 new() 会失败，我们测试 default()
        let decoder = H265Decoder::default();
        assert_eq!(decoder.codec_type(), CodecType::H265);
        assert_eq!(decoder.name(), "H265Decoder");
    }
}
