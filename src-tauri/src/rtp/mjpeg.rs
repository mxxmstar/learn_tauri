...
impl JpegRtpHeader {
    ...
    pub fn data_offset(&self) -> usize {
        if (64..=127).contains(&self.jpeg_type) {
            12
        } else {
            8
        }
    ...
    fn payload_data_offset(&self, payload: &[u8]) -> Option<(usize, Option<QuantizationTables>)> {
        let mut offset = self.data_offset();
        if payload.len() < offset {
            return None;
        }

        if self.offset == 0 && self.q >= 128 {
            if payload.len() < offset + 4 {
                return None;
            }

            let quant_len = u16::from_be_bytes([payload[offset + 2], payload[offset + 3]]) as usize;
            offset += 4;

            if payload.len() < offset + quant_len {
                return None;
            }

            let quant_tables = parse_quantization_tables(&payload[offset..offset + quant_len]);
            offset += quant_len;

            Some((offset, quant_tables))
        } else {
            Some((offset, None))
        }
    }
...
fn parse_quantization_tables(data: &[u8]) -> Option<QuantizationTables> {
    if data.is_empty() {
        return None;
    }

    let mut tables = QuantizationTables {
        luma: None,
        chroma: None,
    };
    ...
    if data.len() >= 64 {
        let mut luma = [0u8; 64];
        luma.copy_from_slice(&data[..64]);
        tables.luma = Some(luma);
    }

    if data.len() >= 128 {
        let mut chroma = [0u8; 64];
        chroma.copy_from_slice(&data[64..128]);
        tables.chroma = Some(chroma);
    }

    if tables.luma.is_some() || tables.chroma.is_some() {
        Some(tables)
    } else {
        None
    }
...
impl MjpegReassembler {
    ...
    pub fn push_packet(&mut self, packet: &RtpPacket) -> Option<JpegFrame> {
        ...
        let (data_offset, quant_tables) = jpeg_header.payload_data_offset(&packet.payload)?;
        ...
        let entry = self
            .frames
            .entry(timestamp)
            .or_insert_with(|| (jpeg_header.clone(), quant_tables.clone(), BTreeMap::new()));

        if entry.1.is_none() {
            entry.1 = quant_tables;
        }

        // 存储此分片
        entry.2.insert(jpeg_header.offset, jpeg_data.to_vec());
    ...
    fn reassemble_frame(&mut self, timestamp: u32) -> Option<JpegFrame> {
        if let Some((jpeg_header, quant_tables, fragments)) = self.frames.remove(&timestamp) {
            // 按 offset 顺序拼接数据
...
mod tests {
    ...
    fn test_mjpeg_reassembler() {
        ...
        let payload1 = Bytes::from(vec![
            ...
            0x00, 0x50, 0x28, 0x1E, 0xAA, 0xBB, 0xCC, // JPEG data part 1
        ]);

        let payload2 = Bytes::from(vec![
            0x00, 0x00, 0x00, 0x03, // JPEG RTP header, offset = 3
            0x00, 0x50, 0x28, 0x1E, 0xDD, 0xEE, 0xFF, // JPEG data part 2
        ]);

        let make_packet = |payload: Bytes, marker: bool| RtpPacket {
            header: RtpHeader {
                version: 2,
                padding: false,
                extension: false,
        ...
        let packet1 = make_packet(payload1, false);
        let packet2 = make_packet(payload2, true);

        assert!(reassembler.push_packet(&packet1).is_none());
        let frame = reassembler.push_packet(&packet2).unwrap();

        assert_eq!(frame.timestamp, 1000);
        assert_eq!(frame.jpeg_header.width, 320);
        assert_eq!(frame.jpeg_header.height, 240);
        assert_eq!(&frame.data[..], &[0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF]);
